...
// Table for Sector size and offset using the sec_t struct, all numbers are bytes.


uint32_t low_to_big32(uint32_t x) {
	uint8_t *p = (uint8_t *) &x;
	return p[0] + (p[1] << 8) + (p[2] << 16) + (p[3] << 24);
}

uint16_t low_to_big16(uint16_t x) {
	uint8_t *p = (uint8_t *) &x;
	return p[0] + (p[1] << 8);
}

uint32_t big_to_low32(uint32_t x) {
	uint8_t *p = (uint8_t *) &x;
	return p[3] + (p[2] << 8) + (p[1] << 16) + (p[0] << 24);
}

uint16_t big_to_low16(uint16_t x) {
	uint8_t *p = (uint8_t *) &x;
	return p[3] + (p[2] << 8);
}
...
typedef struct {
	...
	directory_t *filetable[CONFIG_MAX_OPEN_FILES];
...
uint32_t fat32_fat_lookup(fat32_t *fat, uint32_t cluster) {
	...
	if (r == 0) {
		...
		kprintf(
				"fat32_fat_lookup: Error during disk read. Initialization failed.\n");
		return -1;
	}
...
int next_dir_entry(fat32_t *fat, directory_t *entry) {
	uint32_t addr;
	int r;
	gbd_request_t req;
	...
	do {
		...
		stringcopy(entry->name, (char *) (addr + (entry->entry * 32)), 11);
		if (entry->name[0] == 0)
			return -1;

		entry->attr = DATA_GET(uint32_t, addr, (entry->entry * 32) + 0x0B);
	} while ((uint8_t) entry->name[0] == FAT32_DIR_ENTRY_UNUSED || (entry->attr
			& 0x1111));

	entry->high_cluster
			= DATA_GET(uint16_t *, addr, (entry->entry * 32) + 0x14);
	entry->low_cluster = DATA_GET(uint16_t *, addr, (entry->entry * 32) + 0x1A);
...
int search_dir(fat32_t *fat, directory_t *entry, int(*pred)(directory_t *entry)) {
...
int search_dir_by_filename(fat32_t *fat, directory_t *entry, const char *name) {
	while (next_dir_entry(fat, entry) >= 0) {
		if (stringcmp(entry->name, name) == 0) {
			return VFS_OK;
		}
	}
...
int is_volume_id(directory_t *entry) {
	return (entry->attr & 8);
}
...
fs_t *fat32_init(gbd_t *disk) {
	...
	if (low_to_big16(DATA_GET(uint16_t, addr, FAT32_SIG_OFFSET))
			!= FAT32_SIGNATURE) {
		semaphore_destroy(sem);
		pagepool_free_phys_page(ADDR_KERNEL_TO_PHYS(addr));
		return NULL;
	...
	reserved_sector_count = low_to_big16(
			DATA_GET(uint16_t, addr, FAT32_RESERVED_SECTOR_COUNT_OFFSET));
	secs_per_fat = low_to_big32(
			DATA_GET(uint32_t, addr, FAT32_SECS_PER_FAT_OFFSET));
	num_fats = DATA_GET(uint8_t, addr, FAT32_NUM_FATS_OFFSET);

	fat->sectors_per_cluster
			= DATA_GET(uint8_t, addr, FAT32_SECS_PER_CLUS_OFFSET);
	fat->root_dir_first_cluster = low_to_big32(
			DATA_GET(uint32_t, addr, FAT32_ROOT_CLUS_OFFSET));
	fat->fat_begin_lba = FAT32_MBR_SIZE + reserved_sector_count;
	fat->cluster_begin_lba = FAT32_MBR_SIZE + reserved_sector_count + (num_fats
			* secs_per_fat);

	memoryset(fat->filetable, 0, sizeof(directory_t *) * CONFIG_MAX_OPEN_FILES);
...
int fat32_open(fs_t *fs, char *filename) {
	...
	fat = (fat32_t *) fs->internal;
	direntry = (directory_t*) pagepool_get_phys_page();
	...
	for (i = 0; i < CONFIG_MAX_OPEN_FILES; ++i) {
		if (fat->filetable[i] != NULL) {
			...
			if (search_dir_by_filename(fat, direntry, filename)
					== VFS_NOT_FOUND) {
				semaphore_V(fat->lock);
				return VFS_NOT_FOUND;
			}
...
int fat32_read(fs_t *fs, int fileid, void *buffer, int bufsize, int offset) {
	...
	cluster = (uint32_t) direntry->low_cluster
			+ ((uint32_t) direntry->high_cluster << 4);
